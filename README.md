...
| `/domain/technical` | **Technical city**: isometric SVG island, 5 clickable buildings, drag (or touch-drag, on any screen) to pan |
| `/domain/music` | **Music city (phase 2C)**: same engine, amber theme, 4 buildings (stage, studio, hall, vinyl) |
...
- The city JS + CSS load via `next/dynamic` on every screen size (Phase 3F) — see PERFORMANCE.md for the trade-off.
...
## Phase 3F (the city on phones)

Every domain page now shows the illustrated city on every screen size, not just md+ (768px). The
camera (`useCityCamera.ts`) already handled touch drag correctly, so removing the old gate in
`components/domain/DomainScene.tsx` was the whole change; the minimum camera scale also dropped from
