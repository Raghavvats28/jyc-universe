...
export function useCityCamera(opts: CameraOptions) {
  ...
  const measure = useCallback(() => {
    ...
    // Floor lowered from 0.6 so a narrow phone viewport still fits most of the island without
    // constant panning (a 1460px-wide frame at 0.6 needs an 875px-wide screen just to match it).
    const s = clamp(Math.min(vw / frame.width, vh / frame.height), 0.34, 1.3);
